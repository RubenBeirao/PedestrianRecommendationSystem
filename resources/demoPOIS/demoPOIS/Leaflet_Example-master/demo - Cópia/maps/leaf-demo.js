...
$.getJSON('http://quasar.ptws.net/isctelisboa/wspois.php/point_of_interest', function(data) {
  ...
  var jsonObj = JSONArray.parse(data); 
for ( var i=0; i < jsonObj.length; ++i )
{
 L.marker( [jsonObj[i].latitude, jsonObj[i].longitude], {icon: myIcon} )
  .bindPopup( '<a href="' + jsonObj[i].point_url + '" target="_blank">' + jsonObj[i].point_name + '</a>' )
  .addTo(pois);
}
});
/*
