...
public class GraphhopperServer {
	 ...
	 * @param request
	 */
	public GraphhopperServer(RouteRequest request) {
		// configurations
	...
	public List<PathWrapper> compareAlternativeRoutes(LinkedList<LinkedList<PathWrapper>> pathAlternatives) {
		...
		for (LinkedList<PathWrapper> diffPaths : pathAlternatives) {
			...
			for (PathWrapper path : diffPaths) {
				...
				// CRIAR CÓPIA DO PATHWRAPPER
				// CRIAR ESTRUTURA DOS EDGES DO PATHWRAPPER
				// CICLO DE ITERAÇÃO SOBRE O PATHWRAPPER
				// IR AO MONGO DB BUSCAR CROWDING VALUES DOS EDGES E DISTANCIA DOS EDGES

				// SORT DO CROWDING (QUOCIENTE/RATIO ENTRE 1/(DISTANCIA * CROWDING))
			...
			for (int i = 0; i < ratioValues.size(); i++) {
				double currentRatio = ratioValues.get(i);
				if (currentRatio > maxRatio) {
					maxRatio = currentRatio;
					maxPos = i;
				}
			}
			sortedPaths.add(diffPaths.get(maxPos));
		}
		System.out.println("SORTED PATHS: " + sortedPaths.toString());
		System.out.println("SIZE OF PATHWRAPPER IS: " + sortedPaths.size());
		return sortedPaths;
	}
	...
	public List<PathWrapper> chooseBestDistance(List<PathWrapper> sortedPaths) {
		...
		while (to < sortedPaths.size()) {
			...
			} else if (ratio > bestRatio && effortLevel <= request.getEffortLevel()) {
				best = temp;
				bestRatio = ratio;
				from += numberOfSuggestedPOIs + 1;
				to += numberOfSuggestedPOIs + 1;
	...
	public static void main(String[] args) {
		System.out.println(distFrom(38.7139083968716, -9.133418295136325, 38.713588499581554, -9.133420343699015));
	}
